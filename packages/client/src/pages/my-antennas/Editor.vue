<template>
	<div class="shaynizk">
		<div class="form">
			...
			<Select v-model="src" class="_formBlock">
			...
			<Switch v-model="withReplies" class="_formBlock">{{
				i18n.ts.withReplies
			}}</Switch>
			<Textarea v-model="keywords" class="_formBlock">
				<template #label>{{ i18n.ts.antennaKeywords }}</template>
				<template #caption>{{
					i18n.ts.antennaKeywordsDescription
				}}</template>
			</Textarea>
			<Textarea v-model="excludeKeywords" class="_formBlock">
				<template #label>{{ i18n.ts.antennaExcludeKeywords }}</template>
				<template #caption>{{
					i18n.ts.antennaKeywordsDescription
				}}</template>
			</Textarea>
			<Switch v-model="caseSensitive" class="_formBlock">{{
				i18n.ts.caseSensitive
			}}</Switch>
			<Switch v-model="withFile" class="_formBlock">{{
				i18n.ts.withFileAntenna
			}}</Switch>
			<Switch v-model="notify" class="_formBlock">{{
				i18n.ts.notifyAntenna
			}}</Switch>
		</div>
		<div class="actions">
			<Button inline primary @click="saveAntenna()"
