	...
	<StickyContainer>
		...
		<Spacer :content-max="800">
			...
			>
				...
				>
					<div class="footer">
						<div>
							...
							{{ i18n.ts.createdAt }}:
							<Time :time="page.createdAt" mode="detail" />
						</div>
						<div v-if="page.createdAt != page.updatedAt">
							<i class="ph-alarm ph-bold"></i>
							{{ i18n.ts.updatedAt }}:
							<Time :time="page.updatedAt" mode="detail" />
						</div>
					</div>
					<div class="_block main">
						...
						<div class="actions">
							...
							<div class="user">
								<Avatar :user="page.user" class="avatar" />
								<div class="name">
									<UserName
										:user="page.user"
										style="display: block"
						...
						<!-- <div class="links">
						<A :to="`/@${username}/pages/${pageName}/view-source`" class="link">{{ i18n.ts._pages.viewSource }}</A>
						<template v-if="$i && $i.id === page.userId">
							<A :to="`/pages/edit/${page.id}`" class="link">{{ i18n.ts._pages.editThisPage }}</A>
				...
				<Error v-else-if="error" @retry="fetchPage()" />
				<Loading v-else />
			</transition>
		</Spacer>
	</StickyContainer>
