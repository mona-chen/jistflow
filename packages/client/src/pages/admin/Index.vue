<template>
	<div ref="el" class="hiyeyicy" :class="{ wide: !narrow }">
		<div v-if="!narrow || currentPage?.route.name == null" class="nav">
			<Spacer :content-max="700" :margin-min="16">
				<div class="lxpfedzu">
					<div class="banner">
						<img
					...
					<Info v-if="thereIsUnresolvedAbuseReport" warn class="info"
						>{{ i18n.ts.thereIsUnresolvedAbuseReportWarning }}
						<A to="/admin/abuses" class="_link">{{
							i18n.ts.check
						}}</A>
					</Info>
					<Info v-if="noMaintainerInformation" warn class="info"
						>{{ i18n.ts.noMaintainerInformationWarning }}
						<A to="/admin/settings" class="_link">{{
							i18n.ts.configure
						}}</A>
					</Info>
					<Info v-if="noBotProtection" warn class="info"
						>{{ i18n.ts.noBotProtectionWarning }}
						<A to="/admin/security" class="_link">{{
							i18n.ts.configure
						}}</A>
					</Info>
					<Info v-if="noEmailServer" warn class="info"
						>{{ i18n.ts.noEmailServerWarning }}
						<A to="/admin/email-settings" class="_link">{{
							i18n.ts.configure
						}}</A>
					</Info>
					<Info v-if="updateAvailable" warn class="info"
					...
					<SuperMenu :def="menuDef" :grid="narrow"></SuperMenu>
				</div>
			</Spacer>
		</div>
...
if (defaultStore.state.showAdminUpdates) {
	os.api("latest-version").then((res) => {
		...
		const tagDate = tag.includes("-")
			? tag.substring(0, tag.indexOf("-"))
			: tag;
		const versionDate = version.includes("-")
