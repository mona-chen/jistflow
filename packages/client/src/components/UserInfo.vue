	...
	<article class="_panel user-card">
		...
		<h3 class="title">
			<Avatar
				...
				:show-indicator="true"
			/>
			<A class="name" :to="userPage(user)">
				<UserName :user="user" :nowrap="true" />
			</A>
			<p class="username">
				<Acct :user="user" />
			</p>
		...
		<div class="status">
			<p>
				<Number :value="user.notesCount" />
				{{ i18n.ts.notes }}
			</p>
			<p>
				<Number :value="user.followingCount" />
				{{ i18n.ts.following }}
			</p>
			<p>
				<Number :value="user.followersCount" />
				{{ i18n.ts.followers }}
			</p>
		</div>
		<div class="buttons">
			<slot>
				<FollowButton v-if="$i && user.id != $i.id" :user="user" />
			</slot>
		</div>
	</article>
</template>
...
import XShowMoreButton from "@/components/ShowMoreButton.vue";
